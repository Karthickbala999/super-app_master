...
function HomeC() {
    ...
    // Dynamic filter options for HomeC
    const filterOptions = useMemo(() => {
        const options = {
            price: ["Under ₹500", "₹500 - ₹1000", "₹1000 - ₹2000", "₹2000 - ₹5000"],
            brand: [],
            discount: ["10% and above", "20% and above", "30% and above", "50% and above"],
        };
        if (allProducts.length > 0) {
            options.brand = [...new Set(allProducts.map(p => p.brand_id?.name || 'Essential'))].slice(0, 10).sort();
        }
        return options;
    }, [allProducts]);
        ...
        <div className="min-h-screen bg-white font-[Assistant,sans-serif]">
            ...
            <main className="pt-20 md:pt-24 pb-8">
                ...
                <div className="bg-white shadow-sm mb-2 overflow-x-auto no-scrollbar">
                    <div className="max-w-[1248px] mx-auto flex justify-between items-center py-3 px-4 md:px-10 min-w-max gap-8 md:gap-0">
                        {categoryIcons.map((cat, idx) => (
                            <div key={idx} className="flex flex-col items-center gap-1 group cursor-pointer shrink-0" onClick={() => {
                                if (cat.slug === 'grocery') { navigate('/home-grocery'); }
                                else if (cat.slug === 'top-offers') { navigate('/home-clothes'); }
                                else { navigate(`/categories/${cat.slug}`); }
                            }}>
                ...
                <div className="max-w-[1440px] mx-auto px-4 md:px-10">
                    <div className="mb-8 rounded-sm overflow-hidden shadow-sm">
                        <Swiper modules={[Autoplay, Pagination, Navigation]} autoplay={{ delay: 3000, disableOnInteraction: false }} pagination={{ clickable: true, dynamicBullets: true }} navigation={true} loop={true} className="h-[250px] md:h-[400px]">
                    ...
                    <div className="bg-white mb-8">
                        <div className="flex items-center justify-between py-6 border-b border-gray-100 mb-6">
                            ...
                            <button className="text-pink-500 border border-gray-200 px-6 py-2 rounded-sm text-sm font-bold hover:bg-gray-50 transition-colors uppercase tracking-widest">VIEW ALL</button>
                        </div>
                        <div className="overflow-x-auto no-scrollbar flex gap-6 pb-4">
                            {allProducts.filter(p => !p.isBestSeller).slice(0, 8).map(product => (
                                <div key={product.id} className="min-w-[150px] md:min-w-[190px] flex flex-col group cursor-pointer" onClick={() => navigate(`/product/${product.id}`)}>
                    ...
                    <div className="mb-12">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                            {categories.map((cat, idx) => (
                                <div key={idx} className="relative aspect-[3/4] cursor-pointer group overflow-hidden" onClick={() => navigate(cat.route)}>
                    ...
                    <div className="bg-white pb-20">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
                            {allProducts.slice(0, 10).map(product => (
                                <div key={product.id} className="group cursor-pointer flex flex-col" onClick={() => navigate(`/product/${product.id}`)}>
                                    <div className="aspect-[3/4] relative bg-[#f9f9f9] overflow-hidden mb-3">
                                        ...
                                        <div className="absolute top-2 left-2 bg-pink-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity">WISHLIST</div>
                                    </div>
                                    <div className="">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            ...
                                            <span className="text-pink-500 text-[11px] font-bold">({Math.round(((product.price - product.discountedPrice) / product.price) * 100)}% OFF)</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                ...
                {showFilters && (
                    <FilterModal
                        ...
                        onApply={(filters) => { setSelectedFilters(filters); setShowFilters(false); }}
...
function FilterModal({ onClose, onApply, filterOptions, selectedFilters: initialFilters }) {
    ...
    const [expandedCategories, setExpandedCategories] = useState(['price', 'brand']);
    ...
    const toggleFilter = (filter) => { setSelectedSubFilters((prev) => prev.includes(filter) ? prev.filter((f) => f !== filter) : [...prev, filter]); };
    return (
            ...
            <div className="bg-white w-full max-w-lg rounded-t-[32px] sm:rounded-[32px] max-h-[90vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-300">
                <div className="flex items-center justify-between px-6 py-5 border-b sticky top-0 bg-white z-10">
                    <div className="flex items-center gap-4">
                        <button onClick={onClose} className="p-1"><X size={24} className="text-gray-900" /></button>
                        <h2 className="text-xl font-black text-gray-900 uppercase tracking-tighter">Filters</h2>
                    </div>
                    <button onClick={() => setSelectedSubFilters([])} className="text-pink-600 text-sm font-black uppercase tracking-widest px-3 py-1 rounded hover:bg-pink-50 transition-colors">Clear All</button>
                </div>
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    {categories.map((cat) => (
                        <div key={cat} className="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                            <button onClick={() => toggleCategory(cat)} className={`w-full flex items-center justify-between p-4 text-left transition-colors ${expandedCategories.includes(cat) ? 'bg-pink-50/50' : 'bg-white'}`}>
                                <span className="text-sm font-black uppercase tracking-tight text-gray-800">{cat}</span>
                                {expandedCategories.includes(cat) ? <ChevronUp size={20} className="text-pink-600" /> : <ChevronDown size={20} className="text-gray-400" />}
                            </button>
                            {expandedCategories.includes(cat) && (
                                <div className="p-4 bg-white grid grid-cols-2 gap-3 animate-in fade-in zoom-in-95 duration-200">
                                    {filterOptions[cat]?.map((option) => (
                                        <button key={option} onClick={() => toggleFilter(option)} className={`text-[10px] font-bold py-3 px-4 rounded-xl border-2 transition-all flex items-center justify-center text-center leading-tight ${selectedSubFilters.includes(option) ? 'bg-pink-600 border-pink-600 text-white shadow-lg shadow-pink-100' : 'bg-white border-gray-100 text-gray-600 hover:border-pink-200'}`}>
                                            {option}
                                        </button>
                                    ))}
                                </div>
                ...
                <div className="p-6 border-t bg-gray-50/30 grid grid-cols-2 gap-4">
                    <button onClick={onClose} className="w-full py-4 border-2 border-gray-200 rounded-2xl text-gray-700 font-black text-xs uppercase tracking-widest bg-white active:scale-95 transition-transform">Discard</button>
                    <button onClick={() => onApply(selectedSubFilters)} className="w-full py-4 bg-pink-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-pink-100 active:scale-95 transition-all">Apply</button>
                </div>
            </div>
        </div>
    );
